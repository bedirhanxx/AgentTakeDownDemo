...
namespace PathCreation {
    public class PathCreator : MonoBehaviour {
        ...
        public void ReversePath() // >0 Original, <0 Reverse
        {
            Vector3 point;
            int count = bezierPath.NumPoints;
            for (int i = 0; i < count / 2; i++)
            {
                point = bezierPath.GetPoint(count - (i + 1));
                bezierPath.SetPoint(count - (i + 1), bezierPath.GetPoint(i));
                bezierPath.SetPoint(i, point);
            }
            EditorData.UpdateVertexPath();
        }
#endif
